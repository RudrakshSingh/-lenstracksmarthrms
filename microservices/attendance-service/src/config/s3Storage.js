...
const initializeS3Storage = async () => {
  try {
    ...
    if (AWS_ACCESS_KEY_ID && AWS_SECRET_ACCESS_KEY) {
      ...
      logger.info('Initializing AWS S3 with IAM role (EC2/ECS)...');
      // AWS SDK will automatically use IAM role credentials in EC2/ECS
    }
...
const uploadToS3 = async (fileBuffer, fileName, contentType) => {
  try {
    ...
    // Generate unique file key with timestamp and folder structure
    const timestamp = Date.now();
    const sanitizedFileName = fileName.replace(/[^a-zA-Z0-9.-]/g, '_');
    const fileKey = `attendance/selfies/${timestamp}-${sanitizedFileName}`;
    ...
    // Or: https://{bucket}.s3-{region}.amazonaws.com/{key} (for some regions)
    ...
    return {
      success: true,
      fileKey,
      blobName: fileKey, // For backward compatibility
      url: s3Url,
      contentType,
      size: fileBuffer.length,
      uploadedAt: new Date()

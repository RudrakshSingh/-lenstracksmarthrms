...
const connectDB = async () => {
  try {
    ...
    if (!isProduction) logger.info('cpp-service: MongoDB connected successfully');
  } catch (error) {
...
app.use((req, res) => {
  ...
  routesInfo.push(`GET /api/cpp/status`);
  routesInfo.push(`GET /api/cpp/health`);
  ...
  res.status(404).json({
    ...
    troubleshooting: {
      ...
      basePath: `All routes are under /api/cpp/`
    }
  });
});
